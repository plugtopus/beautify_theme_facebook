...
var themeL = {
    add: function (name, theme, act, themep) {
        ...
        if (closeb) {
            closeb.addEventListener('click', function (evt) {
                var t = this;
                alertify.theme("color").okBtn("Удалить").cancelBtn("Отмена").confirm("Вы уверены что хотите удалить <b>" + name + "</b>?", function () {
                    themeL.delNow(evt, t)
                });
            }, false);
        }
    ...
    create: function () {
        chrome.storage.sync.get({
            ...
            themeL.goEdit({}, themeL.findName(items["themes"] || {}, "Моя тема"));
        })
    }
};
